...
const AllEntries: React.FC<AllEntriesProps> = ({darkMode}) => {
    ...
    return(
        <section className="grid grid-cols-2 md:grid-cols-4">
            {entries.map((entry: Entry, index: number) => {
                return(
                    <div id={entry.id} key={index}className={darkMode ? "bg-gray-900 shadow-md shadow-gray-500 m-3 p-4 rounded flex flex-col justify-between" :"bg-gray-300 shadow-md shadow-gray-500 m-3 p-4 rounded flex flex-col justify-between"}>
                        ...
                        {entry.scheduled_at !== null && (
                            <time className="text-right text-sm md:text-lg">Scheduled At: {new Date(entry.scheduled_at!.toString()).toLocaleDateString()}</time>
                        )}
                        </section>
